...
namespace SAMemAPI
{
    public struct ProcessMemory
    {
        ...
        public string AsString(int length, Encoding encoding)
        {
            ...
            byte[] value = Read(Process, Address, length);
            int stringLength = value.ToList().FindIndex(b => b == 0);
        ...
        {
            return string.Format("{0}@{1}", Process == null ? "NULL" : Process.ProcessName, Address);
        }
